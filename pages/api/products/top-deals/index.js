...
export default async (req, res) => {
  ...
  const topDealProducts = await httpGet(productsUrl, { isBigCommerce: true });
  if (topDealProducts.status === 401) {
    res.status(401);
    res.json({
      errors: {
  ...
  await Promise.all(
    topDealProducts.data.map(async (product, index) => {
      ...
      return (topDealProducts.data[index] = {
        ...topDealProducts.data[index],
        images: productImages.data,
      });
    })
  );

  return res.json(topDealProducts);
};
